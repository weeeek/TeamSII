...
import {playerBulletStrategy} from 'game/fight/js/Strategy'
import BossBullet from 'game/fight/js/BossBullet'
import Plane from 'game/fight/js/Plane'
    ...
    dom.addEventListener('touchend', (e) => {
      if (this.bomb) {
        ...
        while (enemyArr.length > 0) {
          let enemy = enemyArr[enemyArr.length - 1]
          enemy.blood = 0
          let dieEnemy = enemyArr.splice(enemyArr.length - 1, 1)[0]
        ...
        if (boss && boss.state !== 'Appear') {
          ...
          while (boss.bullets.length > 0) {
            let bullet = boss.bullets[boss.bullets.length - 1]
