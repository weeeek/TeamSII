...
import * as $ from 'game/fight/js/Utils'
import {FSM} from 'game/fight/js/FSM'

/* ** 版本 ** */
const version = 'v2.0.1'

/* ** 语言转换 ** */
const lanChangeStrategy = {
  '中文': 'chinese',
  'English': 'english'
}
...
export default class UI {
  ...
  setBtnText (lanSet) { // 设置按钮文本信息
    ...
    for (let i = 0, btn; ;) {
      btn = ['startBtn', 'rankBtn', 'setBtn', 'ruleBtn'][i++]
  ...
  _bindBtnEvent () { // 绑定按钮事件
    ['startBtn', 'rankBtn', 'setBtn', 'ruleBtn'].map((x) => {
      this[x].addEventListener('touchend', () => {
        FSM[this.curState][`click${$.firstUpper(x)}`].call(this)
      })
    })
  }
  ...
  _bindSetEvent () { // 绑定设置相关事件
    ...
    if (!lanSet.onchange) {
      lanSet.onchange = () => {
        ...
        let arr = ['rank', 'set', 'rule']
        arr.map((x) => { this.deleteUI(x) })
        this.setBtnText(this.language)
