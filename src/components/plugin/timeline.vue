    ...
    <div class="wrapper-timeline" v-if="queryTimelineItems.length > 0">
      ...
      >
        <div class="section-year" v-if="timelineContent.items.length > 0">
          ...
          <p
            class="year"
            v-if="timelineContent.show"
            @click="timelineContent.show = !timelineContent.show"
          >
        ...
        <section
          ...
          v-if="timelineContent.items.length > 0"
          v-show="timelineContent.show"
        >
          <a
            ...
            :key="index"
            @click="MQSend(mqType.trip, item.title)"
          >
            ...
            <p
              ...
              v-html="splitDescription(item.description)"
            ></p>
          </a>
        </section>
      </div>
            ...
            x.items.map((y) => {
              ...
              while (index < queryArr.length && allmatch) {
                ...
                if (
                  ...
                  y.description.toLowerCase().includes(queryArr[index])
                ) {
                  // 替换文本
            ...
            x.items.map((y) => {
              ...
              while (index < queryArr.length) {
                if (
                  y.title.toLowerCase().includes(queryArr[index]) ||
                  y.description.toLowerCase().includes(queryArr[index]) ||
                  (y.summarize && y.summarize.includes(queryArr[index]))
                ) {
                  match = true;
                  target.title = target.title.replace(
                    new RegExp(queryArr[index], "gi"),
                    `<span class="keywords">${queryArr[index]}</span>`
                  );
                  target.description = target.description.replace(
                    new RegExp(queryArr[index], "gi"),
